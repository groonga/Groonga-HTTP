...
# Delete load data
{
  ...
  for (my $i = 0; $i < $search_results->{'n_hits'}; $i++) {
    ...
    $records_before_delete[0][$i] = \@record;
  }
  is(
    $records_before_delete[0],
    [
      [
        "The first post!",
  ...
  is(
    ...
    1,
    "successed delete record."
  );

  $search_results = $groonga->select(
     table => 'Entries'
  );
  is(
    $search_results->{'n_hits'},
  ...
  for (my $i = 0; $i < $search_results->{'n_hits'}; $i++) {
    ...
    push (@record, $search_results->{'records'}[$i]->{'n_likes'});
    push (@record, $search_results->{'records'}[$i]->{'tag'});
    $records_after_delete[0][$i] = \@record;
  }
  is(
    $records_after_delete[0],
    [
      [
        "The first post!",
        "Welcome! This is my first post!",
